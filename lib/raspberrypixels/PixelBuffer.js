...
/**
 * Fill each pixels with a specific color. Empty cells have not impact on pixels.
 * @param  {Array} map    Array of pixels to be set up.
 */
PixelBuffer.prototype.fillMapRGB = function(map){
	if(!map || !map.length) {
		// Bad map
		return;
	}
	...
	for(var i = 0; i < map.length; i++){
		if(map[i]) {
			this.buffer[i * 3] = map[i].r;
			this.buffer[i * 3 + 1] = map[i].g;
			this.buffer[i * 3 + 2] = map[i].b;
		}
	}
}
